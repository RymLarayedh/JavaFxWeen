 ...
 */
public class WeenerAdd implements IWeenerAdd {
    ...
    public List<PersonneToPerson> DisplayUserFriends(int id) {
        ...
        List<Integer> listeidfriends = new ArrayList();
        
        listeidfriends=fdao.findById(id);
        for(Integer Y:listeidfriends){
            User u = pdao.findByUserId(Y);
            all.add(u);
        }
        for (User X : all) {
